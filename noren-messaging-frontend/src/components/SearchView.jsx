    ...
    <div className="w-full max-w-2xl mx-auto space-y-6">
      ...
      <form onSubmit={handleSearchSubmit} className="relative">
        ...
        <input
          ...
          className="w-full pl-12 pr-4 py-3.5 text-sm rounded-2xl glass-input border border-white/10"
        />
      </form>
          ...
          {results.hashtags.length > 0 && (
            <div className="glass-card rounded-3xl p-4 border border-white/10 space-y-2 shadow-xl">
              ...
              {results.hashtags.map((h) => (
                <div key={h.id} className="flex items-center justify-between p-2.5 rounded-2xl hover:bg-white/10">
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-2xl bg-white/10 border border-white/20 flex items-center justify-center text-white font-bold">
                      #
                    </div>
                    <div>
                      <h5 className="text-sm font-bold text-white">#{h.tag}</h5>
                      <p className="text-xs text-neutral-400">{h.posts_count} posts</p>
                    </div>
                  </div>
                </div>
              ))}
